...
fun loremIpsumWords(wordCount: Int): String {
    if (wordCount >= loremIpsumWords.size) {
        return loremIpsum
    }

    return buildString {

        if (wordIndex >= loremIpsumWords.size) {
            wordIndex = 0
        }

        for (i in 0 until wordCount) {
            append(loremIpsumWords[wordIndex])
            if (i < wordCount - 1) {
                append(" ")
            }
            wordIndex += 1
